...
4. 将此“糖尿病数据集”数据集从推理管道中删除，并将其替换为“手动输入数据”组件；将该组件连接到与“Web 服务输入”相同的“应用转换”组件的“数据集”输入。     然后，修改“手动输入数据”组件的设置以使用以下 CSV 输入，其中包括对三位新患者观察得出的不带标签的特征值：
