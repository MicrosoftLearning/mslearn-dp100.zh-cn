...
ms.locfileid: "146733072"
---
# <a name="use-azure-machine-learning-designer"></a>使用 Azure 机器学习设计器

Azure 机器学习设计器提供了一个拖放环境，你可以在其中定义工作流或数据引入的管道、转换以及模型训练组件，以创建机器学习模型。  然后，可以将该管道以 Web 服务的形式发布，以便客户端应用程序可以使用该管道进行推理（根据新数据生成预测）。
...
1. 在 Azure 机器学习工作室中，查看“数据”页。 数据集表示计划在 Azure ML 中使用的特定数据文件或表。
...
2. 单击右侧的 &#9881; 图标打开“设置”窗格，将默认管道名称 (*Pipeline-Created-on-date***) 更改为 Visual Diabetes Training。  
3. 请注意，需要指定要在其上运行管道的计算目标。 在“设置”窗格中，单击“选择计算类型”，然后选择“计算群集”，单击“选择 Azure 机器学习计算群集”，选择你的计算群集后，关闭“设置” 。
4. 在设计器的左侧，选择“数据集”选项卡，然后将数据集“糖尿病数据集”拖动到画布上。 
5. 选择画布上的“糖尿病数据集”组件。 然后右键单击它，并选择“预览数据”。
6. 在“DataOutput”窗格中，选择“分析”选项卡。
...
1. 在左侧窗格中，选择“组件”选项卡，其中包含可用于在模型训练之前转换数据的各种组件。 可以在窗格顶部搜索组件。
2. 搜索“规范化数据”组件并将其拖到画布上，位于“糖尿病数据集”组件下方。  然后将“糖尿病数据集”组件的输出连接到“规范化数据”组件的输出。 
3. 选择“规范化数据”组件并查看其设置，请注意，需要指定转换方法和要转换的列。 然后，将转换保留为“ZScore”，编辑各列，使其包含以下列名称：
    * PlasmaGlucose
...
4. 现在，我们可以将数据拆分到多个单独的数据集中，以进行训练和验证。 在左侧窗格中，搜索“拆分数据”组件，并将其拖到画布上，位于“规范化数据”组件下方。  然后将“规范化数据”组件的“转换后的数据集”（左侧）输出连接到“拆分数据”组件的输入。
5. 选择“拆分数据”组件，并按如下所示配置其设置：
    * 拆分模式：拆分行
    * 第一个输出数据集中的行部分：0.7
    * **随机种子**：123
    * **分层拆分**：False

## <a name="add-model-training-components"></a>添加模型训练组件

准备好数据并将其拆分为训练数据集和验证数据集后，即可配置管道来训练和评估模型。

1. 搜索“训练模型”组件，并将其拖到画布上，位于“拆分数据”组件下方。  然后将“拆分数据”组件的“结果数据集 1”（左侧）输出连接到“训练模型”组件的“数据集”（右侧）输入 。
2. 我们训练的模型将预测“Diabetic”值，因此请选择“训练模型”组件，并修改其设置以将“标签列”设置为“Diabetic”（完全匹配大小写和拼写！）   
3. 该模型将预测的“Diabetic”标签是一个二元列（对于有糖尿病的患者，值为 1；对于没有糖尿病的患者，值为 0），因此我们需要使用分类算法来训练模型。 搜索“双类逻辑回归”组件，并将其拖放到画布上，位于“拆分数据”组件左侧、“训练模型”组件上方。   然后将其输出连接到“训练模型”组件的“未训练模型”（左侧）输入。 
4. 为了测试经过训练的模型，我们需要使用它来对拆分原始数据时保留的验证数据集进行评分。 搜索“评分模型”组件并将其拖放到画布上，位于“训练模型”组件下方。  然后将“训练模型”组件的输出连接到“评分模型”组件的“已训练模型”（左侧）输入；并将“拆分数据”组件的“结果数据集 2”（右侧）输出拖动到“评分模型”组件的“数据集”（右侧）输入      。
5. 为了评估模型的性能，我们需要查看对验证数据集进行评分时生成的一些指标。 搜索“评估模型”组件并将其拖到画布上，位于“评分模型”组件下方，然后将“评分模型”组件的输出连接到“评估模型”组件的“评分数据集”（左侧）输入。    
...
3. “规范化数据”组件完成后，选择它，然后在“设置”窗格中的“输出 + 日志”选项卡上，在“已转换的数据集”部分的“数据输出”下，单击“预览数据”图标，并注意你可以查看转换列的统计信息和分布可视化效果     。
4. 关闭“规范化数据”可视化效果并等待其余组件完成。 然后可视化“评估模型”组件的输出，查看模型的性能指标。

    **注意**：此模型的性能不是很好，部分原因是我们仅执行了最少的特征工程和预处理。 可以尝试一些其他分类算法并比较结果（可以将“拆分数据”组件的输出连接到多个“训练模型”和“评分模型”组件，还可以将第二个评分模型连接到“评估模型”组件以便并行比较）。    练习的目的只是为了向你介绍设计器界面，而不是训练完美的模型！

## <a name="create-an-inference-pipeline"></a>创建推理管道

使用训练管道来训练模型后，就可以创建一个推理管道，该推理管道使用经过训练的模型来预测新数据的标签 。

1. 在“作业”选项卡中，导航到已完成的管道。 
2. 选择“创建推理管道”，然后单击“实时推理管道”。 几秒钟后，将打开新版本的管道，名为“视觉对象糖尿病训练-实时推理”。
3. 将新管道重命名为“预测糖尿病”，然后查看新管道。 请注意，标准化转换和经过训练的模型已封装在此管道中，因此训练数据中的统计信息将用于规范化任何新数据值，并且经过训练的模型将用于对新数据进行评分。
4. 请注意，推理管道会假设新数据将与原始训练数据的架构匹配，因此将训练管道中的“糖尿病数据集”数据集包括在内。 然而，此输入数据包括了模型预测的“Diabetic”标签，这对于尚未进行糖尿病预测的新患者数据来说并不具有说服力。
5. 将此“糖尿病数据集”数据集从推理管道中删除，并将其替换为“手动输入数据”组件；将该组件连接到与“Web 服务输入”相同的“应用转换”组件的“数据集”输入。     然后，修改“手动输入数据”组件的设置以使用以下 CSV 输入，其中包括对三位新患者观察得出的不带标签的特征值：
...
5. 推理管道包括“评估模型”组件，此组件对于根据新数据进行预测没有用，因此可将其删除。
6. “评分模型”组件的输出包括所有输入特征以及预测的标签和概率分数。 若要将输出限制为仅预测和概率，请删除“评分模型”组件与“Web 服务输出”之间的连接，添加“执行 Python 脚本”组件。   将“评分模型”组件的输出连接到“执行 Python 脚本”的“Dataset1”（最左侧）输入，并将“执行 Python 脚本”组件的输出连接到“Web 服务输出”    。 然后，修改“执行 Python 脚本”组件的设置，以使用以下代码（替换所有现有的代码）：
...
```
> **注意**：将代码粘贴到“执行 Python 脚本”组件后，验证代码是否与上面的代码类似。 Python 中的缩进很重要，如果没有正确复制缩进，组件将会失败。 
...
1. 如果预测糖尿病推理管道尚未完成运行，请等待其完成。 然后将“执行 Python 脚本”组件的“结果数据集”输出可视化，查看输入数据中三个患者观察值的预测标签和概率。 
